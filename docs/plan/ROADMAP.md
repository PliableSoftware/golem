...
_Generated by `golem task index` from `docs/plan/tasks/` — 4 ready, 20 blocked, 136 done. Edit the task documents, not this table._
...
| task | goal | owner | size | discipline | depends on | gate / blocker |
|---|---|---|---|---|---|---|
| [npm-token-set-but-broken](tasks/npm-token-set-but-broken.md) | "`NPM_TOKEN` is set but does not work — so every release attempts a publish and goes red after succeeding" | user | S | code | — | "MITIGATED 2026-09-04 — releases are green again; publishing now needs `vars.NPM_PUBLISH == 'true'`. Closing it fully still needs the account owner to either delete the broken secret or replace it with a working token." |
...
| [R14.2](tasks/R14.2.md) | Provision per-project Buzz agent identities for the persona bench | agent | M | code | R14.3 | Depends on R14.3 shipping harness "golem" first. Also, Buzz's machine-writable agent-config surface (file / REST / Nostr event kind) is unconfirmed — see verification-notes.md §17. Do not start the provisioning code until both are settled; spend any preparatory work on the ACP/config investigation itself. |
| [R14.3](tasks/R14.3.md) | Implement Golem as its own first-class ACP harness in Buzz | agent | L | code | — | The exact Agent Client Protocol (ACP) surface Buzz expects a harness to implement is unconfirmed — see verification-notes.md §17. This is foundational: R14.2 and R14.4 both build on Golem being spawnable as harness "golem", so implement and verify the ACP handshake in isolation (a minimal echo turn) before wiring any persona or orchestrator behavior on top of it. |
| [R14.4](tasks/R14.4.md) | Make Golem itself an addressable orchestrator inside Buzz | agent | L | code | R14.2, R14.3 | Depends on R14.3 (Golem speaking ACP as its own harness) and R14.2 (persona + orchestrator identities provisioned) both shipping first. |
