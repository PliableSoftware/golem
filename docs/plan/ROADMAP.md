...
_Generated by `golem task index` from `docs/plan/tasks/` — 4 ready, 19 blocked, 136 done. Edit the task documents, not this table._
...
| [R14.2](tasks/R14.2.md) | Provision per-project Buzz agent identities for the persona bench | agent | M | code | — | Buzz's machine-writable agent-config surface (file / REST / Nostr event kind) is unconfirmed — see verification-notes.md §17. Do not start the provisioning code until that is nailed down against block/buzz source or an API doc; spend any preparatory work on the ACP/config investigation itself. |
| [R14.3](tasks/R14.3.md) | Make Golem itself an addressable orchestrator inside Buzz | agent | L | code | R14.2 | Depends on R14.2's identities existing, and on confirming the ACP surface Buzz expects a claude harness to implement (verification-notes.md §17) — Golem's own orchestrator needs to register as a Buzz agent, which may need an ACP shim if the existing Claude Code integration doesn't already speak it. |
