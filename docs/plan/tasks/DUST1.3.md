...
gate: "`docs/plan/audit/dust-1/DUST1.3.md` exists on branch `dust/DUST1.3` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
...
- **src:** `src/compression/` (incl. the Headroom pin — imports only in
  `src/compression/headroom-adapter.ts`), `src/prompt/`, `src/pipeline/brevity.ts` (DUST1.1 owns
  the rest of `src/pipeline/`), `src/hooks/post-tool-use.ts` and `src/hooks/post-tool-use/` (the
  CCR swap path), `src/cli/dials.ts`, `src/cli/mcp-compression.ts`
- **wiki:** `concepts/Compression.md`, `concepts/Compression Levels.md`, `concepts/CCR Ref Scope.md`,
  `concepts/Cache Observability.md`
- **spec:** §1.1, §1.2, §3.2, §3.4, §4 (incl. Quality guardrails)
- **Decisions:** 1, 8, 15, 18, 23, 30, 31, 52, 57
- **ADRs:** ADR-0004 (except the `proxy.bypass_all` claim, which is DUST1.1's)
- **Also verify:** no tool call can change pipeline depth (`level` retired, ADR-0004); leftover
  slider-era names in code or docs are dead-or-superseded candidates
...
`docs/plan/audit/dust-1/DUST1.3.md`, containing:
...
`git worktree add ../Golem-DUST1.3 -b dust/DUST1.3 development`. Commit the findings note
