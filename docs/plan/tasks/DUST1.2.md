---
task: DUST1.2
title: "Dust audit — providers, routing, gateways and credentials: classify every documented claim against the code"
state: done
owner: agent
size: M
discipline: review
...
gate: "`docs/plan/audit/dust-1/DUST1.2.md` exists on branch `dust/DUST1.2` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
touches: [docs/plan/audit/dust-1]
created: 2026-09-25
updated: 2026-09-25T23:34:49.395Z
---
...
- **src:** `src/providers/`, `src/credentials/`; `src/cli/gateways.ts`, `src/cli/gateways/`,
  `src/cli/route-resolver.ts`, `src/cli/targets.ts`, `src/cli/upstream-display.ts`,
  `src/cli/models.ts`
- **wiki:** none dedicated — provider/credential claims on pages owned elsewhere go under a
  **Cross-partition claims** heading, unclassified
- **spec:** §2 Architecture Overview (provider/gateway fronting only)
- **Decisions:** 22, 46, 47, 48, 49
- **ADRs:** ADR-0003
- **Also verify:** the `claude-cli` provider retirement (R13.12) left no live path
...
`docs/plan/audit/dust-1/DUST1.2.md`, containing:
...
`git worktree add ../Golem-DUST1.2 -b dust/DUST1.2 development`. Commit the findings note
