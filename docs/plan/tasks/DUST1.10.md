---
task: DUST1.10
title: "Dust audit — portal, team layer, Buzz and remote/hosted sessions: classify every documented claim against the code"
state: done
owner: agent
size: L
discipline: review
...
gate: "`docs/plan/audit/dust-1/DUST1.10.md` exists on branch `dust/DUST1.10` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
touches: [docs/plan/audit/dust-1]
created: 2026-09-25
updated: 2026-09-25T23:35:02.360Z
---
...
- **src:** `src/portal/`, `src/buzz/`, `src/session/`; `src/cli/init-team.ts`,
  `src/cli/team-skills.ts`, `src/cli/devices.ts`
- **wiki:** `concepts/Buzz Integration.md`, `concepts/Device Authentication.md`,
  `concepts/Free and Team Tiers.md`, `concepts/Hosted Session.md`, `concepts/Joined Session.md`,
  `concepts/Session Transport.md`, `concepts/Conversation Store.md`,
  `concepts/Portal Install Contract.md`, `concepts/Project Team Binding.md`, `concepts/Team Layer.md`
- **Decisions:** 59, 60, 61, 63, 64
- **ADRs:** ADR-0006, ADR-0007
- **Portal half:** the portal repo is at `D:/Personal/Projects/Golem`, not here. Audit the harness
  half only; a claim needing the portal side goes under **Unverifiable here**, never guessed.
  Open `portal-*` drift tasks are already known — cite them, do not re-find
...
`docs/plan/audit/dust-1/DUST1.10.md`, containing:
...
`git worktree add ../Golem-DUST1.10 -b dust/DUST1.10 development`. Commit the findings note
