---
task: DUST1.8
title: "Dust audit — config, init, install, release and vibe: classify every documented claim against the code"
state: done
owner: agent
size: L
discipline: review
...
gate: "`docs/plan/audit/dust-1/DUST1.8.md` exists on branch `dust/DUST1.8` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
touches: [docs/plan/audit/dust-1]
created: 2026-09-25
updated: 2026-09-25T23:34:59.160Z
---
...
- **src:** `src/config/`, `src/pkg/`, `src/update/`, `src/vibe/`, `src/shared/`, `src/index.ts`,
  `src/version.ts`; `src/cli/init.ts`, `src/cli/init-claude-settings.ts`, `src/cli/init-error.ts`,
  `src/cli/init-skills.ts`, `src/cli/init-vscode.ts`, `src/cli/config.ts`, `src/cli/local-config.ts`,
  `src/cli/claude-settings-target.ts`, `src/cli/managed-files.ts`, `src/cli/json-file.ts`,
  `src/cli/vscode-extension.ts`, `src/cli/version-sync.ts`, `src/cli/pkg.ts`, `src/cli/verify.ts`,
  `src/cli/verify-progress.ts`, `src/cli/main.ts`, `src/cli/program.ts`; `scripts/release.mjs` and
  `.github/workflows/` (release claims only)
- **wiki:** `concepts/Configuration Surfaces.md`, `concepts/Settings Cascade.md`,
  `concepts/Personal Vibe Guide.md`, `concepts/Release Pipeline.md`
- **spec:** §5.1 configuration (settings hierarchy half)
- **Decisions:** 4, 9, 16, 19, 41, 43, 58, 62
- **ADRs:** ADR-0008
- **Also verify:** CLAUDE.md "Branches and releases" and "Running the tests" against
  `package.json` scripts and the workflows; config keys named in the owned wiki pages exist in
...
`docs/plan/audit/dust-1/DUST1.8.md`, containing:
...
`git worktree add ../Golem-DUST1.8 -b dust/DUST1.8 development`. Commit the findings note
