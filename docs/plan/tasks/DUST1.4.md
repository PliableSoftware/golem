...
gate: "`docs/plan/audit/dust-1/DUST1.4.md` exists on branch `dust/DUST1.4` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
...
- **src:** `src/knowledge/`, `src/wiki/`, `src/hooks/web-fetch.ts`, `src/hooks/web-fetch/`;
  `src/cli/build-knowledge.ts`, `src/cli/auto-index.ts`, `src/cli/distill.ts`,
  `src/cli/distill-note.ts`, `src/cli/promote.ts`, `src/cli/notes.ts`, `src/cli/synthesize.ts`,
  `src/cli/wiki.ts`, `src/cli/watch.ts`
- **wiki:** `concepts/Knowledge Base.md`, `concepts/Wiki-First Knowledge.md`,
  `concepts/Distillation Pipeline.md`, `concepts/Web Cache.md`, `concepts/Auto-Index Cost.md`,
  `concepts/Repo Map.md`, `concepts/LSP Bridge.md`, and `WIKI.md`'s zone/write rules (not its index)
- **spec:** §3.1
- **Decisions:** 2, 3, 13, 17, 24, 28, 29, 42, 44, 54; the KB-retrieval half of 33
- **ADRs:** ADR-0001
...
`docs/plan/audit/dust-1/DUST1.4.md`, containing:
...
`git worktree add ../Golem-DUST1.4 -b dust/DUST1.4 development`. Commit the findings note
