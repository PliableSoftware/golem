---
task: DUST1.6
title: "Dust audit — inference, personas and local models: classify every documented claim against the code"
state: done
owner: agent
size: M
discipline: review
...
gate: "`docs/plan/audit/dust-1/DUST1.6.md` exists on branch `dust/DUST1.6` and classifies EVERY owned wiki page claim, spec section, Decision and ADR below with file:line evidence; every owned src file is cited as evidence or listed under Undocumented / Dead candidates. Zero changes outside that one file."
depends_on: []
touches: [docs/plan/audit/dust-1]
created: 2026-09-25
updated: 2026-09-25T23:34:55.302Z
---
...
- **src:** `src/inference/`; `src/cli/personas.ts`, `src/cli/persona-sync.ts`,
  `src/cli/persona-watcher.ts`, `src/cli/persona-preference-rule.ts`, `src/cli/init-personas.ts`,
  `src/cli/agents.ts`, `src/cli/local-model.ts`, `src/cli/ollama.ts`
- **wiki:** `concepts/Persona Registry.md`, `concepts/Hosted-multi-turn-claude-CLI-spike.md`
- **spec:** §1 Target hardware profiles, §2.2 Device registry & job scheduler, §3.3
- **Decisions:** 5, 6, 7, 12, 25, 26
- **Also verify:** generated `.claude/agents/golem-<id>.md` and the persona-preference rule match
  what `inference.personas` config produces (R13.12, R14.1)
...
`docs/plan/audit/dust-1/DUST1.6.md`, containing:
...
`git worktree add ../Golem-DUST1.6 -b dust/DUST1.6 development`. Commit the findings note
