...
- `golem-planner` — claude-opus-5-5 (plan) — Breaks a non-trivial or ambiguous task down into a concrete implementation plan — critical files, ordering, trade-offs — before code changes begin.
- `golem-reviewer` — claude-opus-5-5 (review) — Reads code as code and reports defects, without the authoring session's assumptions.
