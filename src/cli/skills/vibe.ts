/**
 * `/vibe` — the personal style guide's active half.
 *
 * The PASSIVE half is the `vibe` guidance rule, which is always in context and
 * tells a coding, writing or review turn to consult the guide. This skill holds
 * the verbs that change it: seed it, show it, and confirm what has been noticed.
 *
 * Installed UNPREFIXED, as `.claude/skills/vibe/SKILL.md` -> `/vibe`, which is
 * why `init-skills.ts` carries an allowlist. It is the user's own style, not a
 * Golem feature they invoke, and `/golem-vibe` read wrong for that.
 */
...
/** The one unprefixed skill. Keyed by its literal directory name. */
export const VIBE_SKILLS: Readonly<Record<string, string>> = { vibe };
